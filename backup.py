"""
backup.py — irishpeptides-desktop auto-backup script.
...
2. Zips the project (excl. dist, node_modules) -> backups/project_backup.zip
3. Uploads to Google Drive 'Irish Peptides Backup/irishpeptides-desktop/' folder
...
Encryption key: ~/.ip_jarvis_backup_key  (shared across repos, never commit)
"""
import sys
import os
import json
...
# ── Paths ────────────────────────────────────────────────────────────────────
PROJECT_DIR  = Path(r"C:\Projects\irishpeptides-desktop")
...
DRIVE_SUBFOLDER     = "irishpeptides-desktop"
DRIVE_SCOPES        = ["https://www.googleapis.com/auth/drive.file"]

SKIP_DIRS  = {".git", "__pycache__", "backups", "node_modules", "dist"}
SKIP_EXTS  = {".pyc", ".pyo"}
...
log = logging.getLogger("backup-desktop")
...
def _get_or_create_key() -> bytes:
    ...
    try:
        ...
        pass
    log.info("New encryption key generated -> %s", KEY_FILE)
    return key
...
def _drive_upload_file(service, local_path: Path, folder_id: str, custom_name: str = None) -> bool:
...
def upload_installer_to_drive(installer_path: Path, ts_label: str) -> bool:
    """Upload installer EXE to releases/ subfolder with timestamp in name."""
    service = _get_drive_service()
    if not service:
        log.warning("Drive not authenticated — skipping installer upload")
        return False
    try:
        parent_id  = _get_or_create_folder(service, DRIVE_PARENT_FOLDER)
        sub_id     = _get_or_create_folder(service, DRIVE_SUBFOLDER, parent_id)
        rel_id     = _get_or_create_folder(service, "releases", sub_id)
        stamp_name = f"IrishPeptidesJarvis-Setup-{ts_label}.exe"
        _drive_upload_file(service, installer_path, rel_id, custom_name=stamp_name)
        log.info("Installer uploaded -> releases/%s", stamp_name)
        return True
    except Exception as e:
        log.error("Installer Drive upload failed: %s", e)
        return False
...
def update_backup_status(ts: str) -> None:
    ...
    current["repos"]["irishpeptides-desktop"] = {"last_backup": ts, "status": "ok"}

    content_b64 = base64.b64encode(json.dumps(current, indent=2).encode()).decode()
    payload = {
        "message": f"chore: update irishpeptides-desktop backup status [{ts[:10]}]",
        "content": content_b64,
    }
    if sha:
        payload["sha"] = sha
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            log.info("Backup status updated in GitHub: irishpeptides-desktop = %s", ts)
    except Exception as e:
...
def auth_drive() -> None:
    ...
    if not CREDS_FILE.exists():
        print(f"\nMissing credentials: {CREDS_FILE}\nSee irishpeptides-jarvis/backup.py --auth-drive\n")
        sys.exit(1)
...
def run_backup() -> None:
    ...
    log.info("=== irishpeptides-desktop backup started — %s ===", ts)
    ...
    # Write local status
    (BACKUP_DIR / "last_backup.json").write_text(
        json.dumps({"last_backup": ts, "repo": "irishpeptides-desktop"})
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="irishpeptides-desktop backup")
    parser.add_argument("--auth-drive", action="store_true")
    parser.add_argument("--upload-installer", metavar="PATH",
                        help="Upload installer EXE to Drive releases/ with timestamp")
    args = parser.parse_args()

    if args.auth_drive:
        auth_drive()
    elif args.upload_installer:
        ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        upload_installer_to_drive(Path(args.upload_installer), ts)
    else:
        run_backup()
